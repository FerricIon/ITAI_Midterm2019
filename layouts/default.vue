  ...
  data() {
    return {
      ...
      info: {
        ...
        recommendation2: '<p>协同过滤主要是基于“人-物-关联”的数据信息来寻求“人-人”或“物-物”的联系，从而达到优质推荐的目的。在本次项目中，我们处理的数据信息形式为“用户-电影-评分”；面对这样的现有数据，我们可以基于不同对象，有两种选择：</p><ul><li>基于用户：寻求用户之间的关联。以两个用户对各部电影给分的一致与否作为关联的标准，用数据库生成用户形象分类。进行推荐时，系统根据输入的电影生成当前的用户形象，并用户形象类别中进行匹配，最后返回该类普遍给分较高的电影；</li><li>基于电影：寻求电影之间的关联。通过两部电影获得各用户给分的一致与否作为关联标准，用数据库生成“电影-电影”的关联度矩阵。进行推荐时，系统根据输入的多部电影寻求各自关联度最高的多部电影，最后进行综合排序取优。</li></ul><p>然而协同过滤也有它的局限性，不能只靠它来进行推荐：由于用户对电影的评价很大程度与影片质量有关，因此质量相近但风格迥异的电影很有可能也会收获相近的评分，而生成的用户类群也会不甚明晰。因此我们选择先“以物为本”，把第一节“基于内容”评价方式中的数据进行再加工，将“电影-电影”内容属性的相似度提炼出来作为匹配的硬性标准；然后再利用协同过滤这个强有力的人性化推荐手段，让推荐结果不受限于标签和属性的计数，更受到上千万条由真人用户给出的“用户-电影-评分”数据的影响。</p><h4 id="">实施步骤</h4><ol><li>首先，我们利用“用户-电影-评分”数据库计算出了每个用户的平均打分，再将某个用户对某个电影的评分减去该用户的平均给分，将此结果作为该用户对该电影的喜好程度，从而矫正每个用户总体打分偏高或偏低带来的评分误差。</li><li>然后，我们对每两部电影进行分析，将两者共有的打分用户作为参照，把每个用户的给分作为一个维度，在生成的抽象空间中计算两部电影对应的两个抽象向量的夹角，以它的余弦值作为协同过滤生成的该两部电影的关联度。</li><li>最后，为了缩短网页查询时间，在处理“基于内容”评价方式所得数据库时我们只保留了每部电影关联度最高的40部电影，再对这个5000×40的“电影-电影”矩阵中的每个关联度添上我们通过协同过滤所得到的相应“电影-电影”关联度，最终进行关联度的求和、排序，即可得到推荐的电影。</li></ol><center><img src="/formula/1.png" width="60%"></center><p>其中 <img src="/formula/2.png" width="40px"> 表示用户 <img src="/formula/u.png" width="12px"> 对电影 <img src="/formula/3.png" width="20px"> 的评分；<img src="/formula/4.png" width="20px"> 表示用户 <img src="/formula/u.png" width="12px"> 的平均评分。</p>',
        rate: '<p>通过训练简单的神经网络对输入给出评分。目前效果比较差，待修改。</p>'
      }
    }
  },
  mounted() {
