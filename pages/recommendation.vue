  ...
  v-flex(xs8)
    v-container(grid-list-md, fill-height, fluid)
      v-layout(align-center, justify-center, fill-height, wrap)
        ...
        v-flex(v-if="!form1.loadingResult && form1.result.length" v-for="(result, ind) in form1.result", :key="ind", xs3)
          v-card(height="150px")
            ...
            v-card-content(v-if="result")
              v-rating(readonly, half-increments, length="10", dense, small, :value="result.voteAverage")
  v-flex(xs12) a
</template>
